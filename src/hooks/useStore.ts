...
function docToDb(d: Document, uid: string) {
  return {
    ...
    amount_paid: d.amountPaid, notes: d.notes, terms_and_conditions: d.termsAndConditions, payment_method: d.paymentMethod,
  ...
  const addDocument = useCallback((type: DocumentType, clientId: string) => {
    ...
    const next = mutate(prev => {
      ...
      doc = {
        ...
        vatRate: biz.vatRate, vatAmount: 0, total: 0, amountPaid: 0,
